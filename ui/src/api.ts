...
import {
  ...
  type AccessControlPolicySnapshot,
...
export const api = {
  ...
  videoStreamUrl: (id: string) => `/api/videos/${id}/videostream`,
  ...
  updateYtdlp: () => http<{ ok: boolean; before: string | null; after: string | null; detail: string | null }>(
    "/downloads/ytdlp/update", { method: "POST" }),
  ...
  reorderPermissionGroups: (groupIds: number[]) => http<AccessControlPolicySnapshot>("/access-control/group-order", { method: "PUT", body: JSON.stringify({ group_ids: groupIds }) }),
  deletePermissionGroup: (id: number, replacementGroupId: number) => http<AccessControlPolicySnapshot>(`/access-control/groups/${id}?replacement_group_id=${replacementGroupId}`, { method: "DELETE" }),
  setDefaultPermissionGroup: (groupId: number) => http<AccessControlPolicySnapshot>("/access-control/default-group", { method: "PUT", body: JSON.stringify({ group_id: groupId }) }),
