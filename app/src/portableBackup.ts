...
export const BACKUP_SECTIONS: readonly BackupSectionDefinition[] = [
  ...
  { id: "profile.playlists", schemaVersion: 3, scope: "profile", sensitivity: "normal", dependencies: ["profiles.index", "library.referenced-videos"], category: "organization", path: profilePath("playlists.jsonl") },
...
async function sectionData(id: string, profile: any | null, referenced: Set<string>, exportProfiles: any[] = []): Promise<unknown | unknown[]> {
  ...
  switch (id) {
    ...
    case "profile.playlists": return Promise.all((await database.prepare("SELECT id, portable_uuid, name, icon, sort_order, created_at, offline_policy FROM user_playlists WHERE user_id=?").all(uid) as any[]).map(async (playlist) => ({ uuid: playlist.portable_uuid, name: playlist.name, icon: playlist.icon, order: playlist.sort_order, createdAt: playlist.created_at, offlinePolicy: playlist.offline_policy, videos: (await database.prepare("SELECT video_id, added_at, position FROM user_playlist_videos WHERE playlist_id=? ORDER BY position, video_id").all(playlist.id) as any[]), rules: await database.prepare("SELECT pattern, match_type, field FROM user_playlist_rules WHERE playlist_id=?").all(playlist.id) })));
      ...
      for (const profile of state.manifest.profiles) {
        ...
        if (selected.has("profile.playlists")) {
          ...
          for (const row of get("profile.playlists") ?? []) {
            ...
            const existingPolicy = playlistId ? (await database.prepare("SELECT offline_policy FROM user_playlists WHERE id=? AND user_id=?").get(playlistId,uid) as { offline_policy: string } | null)?.offline_policy : null;
            const offlinePolicy=["none","download","keep"].includes(row.offlinePolicy)?row.offlinePolicy:(existingPolicy??"none");
            if (!playlistId) playlistId=Number((await database.prepare("INSERT INTO user_playlists(name,icon,sort_order,created_at,user_id,portable_uuid,offline_policy) VALUES(?,?,?,?,?,?,?) RETURNING id").run(row.name,row.icon??"ListMusic",row.order??0,row.createdAt??new Date().toISOString(),uid,row.uuid,offlinePolicy)).lastInsertRowid);
            else await database.prepare("UPDATE user_playlists SET name=?,icon=?,sort_order=?,offline_policy=? WHERE id=? AND user_id=?").run(row.name,row.icon,row.order,offlinePolicy,playlistId,uid);
